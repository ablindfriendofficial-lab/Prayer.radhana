...
async function init() {
    try {
        ...
        client.auth.onAuthStateChange((_event, session) => handleAuthState(session?.user || null));
    } catch (err) {
        console.error(err);
    ...
    data.forEach(p => {
        ...
        card.innerHTML = `
            ...
            <div style="display:flex; gap:8px; margin-top: auto;">
                ...
                <button class="btn-base btn-danger btn-p-del">🗑</button>
            </div>
        `;
        card.querySelector('.btn-p-run').onclick = () => runPreview(p.html, p.css, p.js);
...
el('btn-close-preview').onclick = () => setView('view-editor');
el('btn-run-code').onclick = () => runPreview(el('edit-html').value, el('edit-css').value, el('edit-js').value);
